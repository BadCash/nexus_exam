...
class CMContent extends CObject implements IHasSQL, ArrayAccess, IModule {
  ...
  public static function SQL($key=null, $args=null) {
    ...
    $queries = array(
      ...
      'select * by user and type' => "SELECT c.*, u.acronym as owner FROM Content AS c INNER JOIN User as u ON c.idUser=u.id WHERE type=? AND idUser=? AND deleted IS NULL ORDER BY {$order_by} {$order_order};",
      'select * by user' 		  => "SELECT c.*, u.acronym as owner FROM Content AS c INNER JOIN User as u ON c.idUser=u.id WHERE idUser=? AND deleted IS NULL ORDER BY {$order_by} {$order_order};",
  ...
  public function ListAll($args=null) {    
    try {
	  if(isset($args) && isset($args['idUser']) && isset($args['type']) ) {
        return $this->db->ExecuteSelectQueryAndFetchAll(self::SQL('select * by user and type', $args), array($args['type'], $args['idUser']));
	  }
	  elseif(isset($args) && isset($args['idUser']) && !isset($args['type']) ) {
        return $this->db->ExecuteSelectQueryAndFetchAll(self::SQL('select * by user', $args), array($args['idUser']));
	  }
  ...
  public static function Filter($data, $filter) {
	$data = nl2br( CTextFilter::Filter( $data, array($filter, 'clickable') ) );
/*
    switch($filter) {
