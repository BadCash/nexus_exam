...
/*
	...
	var_dump( $this->config['controllers'] );
...
*/
  ...
  public function ThemeEngineRender() {
    ...
    $themePath 	= NEXUS_INSTALL_PATH . '/' . $this->config['theme']['path'];
    ...
    if(isset($this->config['theme']['parent'])) {
      $parentPath = NEXUS_INSTALL_PATH . '/' . $this->config['theme']['parent'];
      $parentUrl	= $this->request->base_url . $this->config['theme']['parent'];
    }
    
    // Add stylesheet name to the $nx->data array
    $this->data['stylesheet'] = $this->config['theme']['stylesheet'];
    
    // Make the theme urls available as part of $nx
    ...
    $nx = &$this;
    // First the default Nexus themes/functions.php
  ...
  /**
   * Draw HTML for a menu defined in $nx->config['menus'].
   *
